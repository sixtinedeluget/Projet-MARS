...
# Découpage en blocs
blocs_cv1 = decouper_cv_en_blocs(clean_textcv1)

# UN SEUL modèle (le bon)
model = SentenceTransformer('all-mpnet-base-v2')

# Un vecteur par bloc
embedding_competences = model.encode(blocs_cv1["competences"])
embedding_experiences = model.encode(blocs_cv1["experiences"])
embedding_formations = model.encode(blocs_cv1["formations"])

# Fusion des 3 blocs en un seul vecteur CV
embeddingscv1 = np.concatenate([
    embedding_competences,
    embedding_experiences,
    embedding_formations
])
